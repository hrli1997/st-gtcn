...
tcn_input_channel = 100
# tcn_channels = [102, 300, 200, 100, 32, 2]
tcn_channels = [tcn_input_channel, 100, 64, 32, 16, 2]
...
def train(batch_count, centres, num_class, model_G, model_D, img, GRU_num_layers,
    ...
    g_optimizer = torch.optim.Adam(model_G.parameters(), lr=0.0001, weight_decay=1e-6)
    ...
    for epoch in range(num_epoch):  # 进行多个epoch的训练
        ...
        for cnt, batch in enumerate(loader_train):
            ...
            traj_pred, context = model_G(V_obs, A_obs, img, centres)
            ...
            # 这样就达到了对抗的目的
            # 计算假的轨迹的损失
            traj_pred, _ = model_G(V_obs, A_obs, img, centres)
            ...
            if (cnt + 1) % batch_size == 0:
                ...
                time_end = time.time()
                print('Batch/Epoch[{}/{}], used_time:{:.3f}'.format(
                    cnt, epoch, time_end - time_start  # 打印的是真实轨迹的损失均值
                ))
                time_start = time.time()
                print('d_loss:{:.6f}, g_loss:{:.6f}, '
                    ...
                    batch_loss_G_traj, batch_loss_G_class))
